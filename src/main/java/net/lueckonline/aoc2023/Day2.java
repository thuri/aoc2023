...
public class Day2 {
	...
	public static void main(String[] args) {
		...
		final var result = Arrays.stream(INPUT.split("\\n"))
		.map(Game::new)
		.reduce(0, (sum, game) -> sum + (bag.isValidGame(game) ? game.id : 0), (sum1,sum2) ->  sum1+sum2)
		;
	...
	static class Bag {
		...
		final Map<Color,Integer> map = Map.of(
			Color.red,   12,
			Color.green, 13,
			Color.blue,  14
		);
	...
	static final class CubeSet {
		...
		static final Pattern pattern = Pattern.compile("^(\\d+)\\s+(green|blue|red)$");
		...
		CubeSet (String serialized) {
			final var parts = serialized.trim().split("\\s",2);
